"""
Import 50 zadataka za kolegij "Diferencijalni racun".

Pokretanje (iz mape backend/):
    python -m app.import_zadaci_diff
"""
...
ZADACI = [
    ...
    },
    {
        ...
        "hints": [r"Zapišite $a_n = 2 + \tfrac{1}{n+2}$.", r"Niz je padajući — prvi član je max/sup, limes je inf."]
    },
    {
    ...
    {
        "tekst": r"Tri broja čine geometrijski niz. Ako drugi uvećamo za 8, dobivamo aritmetički niz. Ako u tom nizu posljednji uvećamo za 64, opet dobivamo geometrijski niz. Odredite te brojeve.",
        "tip": "self",
        "tocan_odgovor": "4, 12, 36",
        "rjesenje": r"Neka su $a, aq, aq^2$. Iz uvjeta: $q=3, a=4$. Brojevi su $4, 12, 36$.",
        "hints": [r"Postavite $a(q-1)^2 = 16$.", r"Iz sustava: $q=3, a=4$."]
    },
    {
    ...
    },
    {
        ...
        "hints": [r"$a_3 a_9 = a_1^{2}q^{10} = 9$.", r"$P_{11} = 9^{11/2} = 3^{11}$."]
    },
    {
        "tekst": r"Za $\epsilon = 0.02$ odredite $n_0(\epsilon)$ iz definicije limesa niza $$\lim_{n \to \infty} \frac{n-1}{2n+1} = \frac{1}{2}$$",
        "tip": "auto",
        "tocan_odgovor": "38",
        "rjesenje": r"$\tfrac{3}{2(2n+1)} < 0.02 \Rightarrow n > 37$. Za $n=37$ je jednako, ne strogo manje. $n_0 = 38$.",
    ...
    },
    {
        ...
        "tip": "auto",
        "tocan_odgovor": "1",
        "rjesenje": r"Baza $= 1+\tfrac{2}{2n^{2}-1}$. Produkt u eksponentu $\to 0$. Limes je $e^0 = 1$.",
        "hints": [r"Napišite bazu kao $1 + \tfrac{2}{2n^{2}-1}$.", r"Produkt teži k $0$, limes je $1$."]
    },
    {
    ...
    {
        "tekst": r"Izračunajte: $$\lim_{n \to \infty} \frac{1 + \tfrac{1}{4} + \tfrac{1}{4^{2}} + \cdots + \tfrac{1}{4^{n}}}{\tfrac{1}{7} + \tfrac{1}{7^{2}} + \cdots + \tfrac{1}{7^{n}}}$$",
        "tip": "auto",
        "tocan_odgovor": "8",
    ...
    },
    {
        ...
        "hints": [r"Konjugatom: $\tfrac{(2-\lambda)n+2}{\sqrt{\cdots}+\sqrt{\cdots}}$.", r"Limes $= \tfrac{2-\lambda}{2} = 1 \Rightarrow \lambda=0$."]
    },
    {
        "tekst": r"Odredite domenu funkcije: $$f(x) = \frac{1-\ln 2x}{1+\ln 2x} - \sqrt{\frac{e^{\,1/(2-x)}}{(x-1)^{3}}}$$",
        "tip": "self",
        "tocan_odgovor": "(1, 2) ∪ (2, ∞) \\ {1/(2e)}",
        "rjesenje": r"$x>0$, $x \neq \tfrac{1}{2e}$, $(x-1)^{3}>0 \Rightarrow x>1$, $x \neq 2$. Domena: $(1,2) \cup (2,\infty) \setminus \{\tfrac{1}{2e}\}$.",
        "hints": [r"Treba $x>0$, $x\neq\tfrac{1}{2e}$.", r"$(x-1)^{3}>0 \Rightarrow x>1$, uz $x\neq 2$."]
    },
    {
        "tekst": r"Odredite domenu funkcije: $$f(x) = \sqrt{\log_{1/2} \frac{1-2x}{x+3}}$$",
        "tip": "self",
        "tocan_odgovor": "[-2/3, 1/2)",
        "rjesenje": r"$\log_{1/2}(T) \ge 0 \iff 0 < T \le 1$. Rješavamo $0 < \tfrac{1-2x}{x+3} \le 1$: $x \in [-\tfrac{2}{3}, \tfrac{1}{2})$.",
        "hints": [r"Baza $<1$: $\log_{1/2}(T)\ge 0 \iff 0<T\le 1$.", r"Riješite $0 < \tfrac{1-2x}{x+3} \le 1$."]
    },
    {
        "tekst": r"Odredite domenu funkcije: $$f(x) = \frac{1}{\sqrt{|x-1| - x}}$$",
        "tip": "self",
        "tocan_odgovor": "(-∞, 1/2)",
        "rjesenje": r"$|x-1|>x$. Za $x\ge 1$: nemoguće. Za $x<1$: $1-x>x \Rightarrow x<\tfrac{1}{2}$.",
        "hints": [r"Treba $|x-1|>x$.", r"Za $x<1$: $x<\tfrac{1}{2}$."]
    },
    {
    ...
    {
        "tekst": r"Odredite sva realna rješenja jednadžbe: $$\frac{|x+1|}{|x-2| + |2x-3|} = 1$$",
        "tip": "self",
        "tocan_odgovor": "x = 1 i x = 3",
        "rjesenje": r"$|x+1| = |x-2|+|2x-3|$. Kritične točke: $-1, \tfrac{3}{2}, 2$. Rješenja: $x=1$ i $x=3$.",
        "hints": [r"Ekvivalentno: $|x+1| = |x-2|+|2x-3|$.", r"Rastavite na intervale."]
    },
    {
    ...
    {
        "tekst": r"Odredite polinom $f$ drugog stupnja kojemu je $-3$ dvostruka nultočka i vrijedi $f(1) - 3f(-5) = 8$.",
        "tip": "self",
        "tocan_odgovor": "f(x) = 2(x+3)^2",
        "rjesenje": r"$f(x) = a(x+3)^{2}$. $16a-12a = 4a = 8 \Rightarrow a=2$. $f(x) = 2(x+3)^{2}$.",
        "hints": [r"Oblik: $f(x) = a(x+3)^{2}$.", r"$4a=8 \Rightarrow a=2$."]
    },
    {
    ...
    {
        "tekst": r"Derivirajte funkciju: $$f(x) = (3x^{2}-1)^{\operatorname{ctg}2x}$$",
        "tip": "self",
        "tocan_odgovor": "Logaritamsko deriviranje",
        "rjesenje": r"$\ln y = \operatorname{ctg}2x\cdot\ln(3x^{2}-1)$. $\tfrac{y'}{y} = -\tfrac{2}{\sin^{2}2x}\ln(3x^{2}-1)+\tfrac{6x\operatorname{ctg}2x}{3x^{2}-1}$.",
        "hints": [r"Logaritamsko deriviranje: $\ln y = \operatorname{ctg}2x\cdot\ln(3x^{2}-1)$."]
    },
    {
        "tekst": r"Derivirajte funkciju: $$f(x) = \operatorname{arctg}\sqrt{\log_3 4x}$$",
        "tip": "self",
        "tocan_odgovor": "Lančano pravilo",
        "rjesenje": r"$f'(x) = \tfrac{1}{1+\log_3 4x}\cdot\tfrac{1}{2\sqrt{\log_3 4x}}\cdot\tfrac{1}{x\ln 3}$",
        "hints": [r"Lančano pravilo za kompoziciju arctg, korijena i logaritma."]
    },
    {
        "tekst": r"Derivirajte funkciju: $$f(x) = \frac{e^{3x}+e^{-3x}}{e^{x}-e^{-x}}$$",
        "tip": "self",
        "tocan_odgovor": "Pravilo kvocijenta",
        "rjesenje": r"$f'(x) = \tfrac{3(e^{3x}-e^{-3x})(e^{x}-e^{-x})-(e^{3x}+e^{-3x})(e^{x}+e^{-x})}{(e^{x}-e^{-x})^{2}}$",
        "hints": [r"$(u/v)' = (u'v-uv')/v^{2}$."]
    },
    {
    ...
    {
        "tekst": r"Odredite stranice pravokutnika maksimalnog opsega upisanog u polukrug polumjera $R$.",
        "tip": "self",
        "tocan_odgovor": "a = 4R/√5, b = R/√5",
        "rjesenje": r"$O = 2(2x+\sqrt{R^{2}-x^{2}})$. Iz $O'=0$: $x=\tfrac{2R}{\sqrt{5}}$. $a=\tfrac{4R}{\sqrt{5}}$, $b=\tfrac{R}{\sqrt{5}}$.",
        "hints": [r"Opseg: $O=2(2x+\sqrt{R^{2}-x^{2}})$.", r"$O'=0 \Rightarrow x=\tfrac{2R}{\sqrt{5}}$."]
    },
    {
        ...
        "rjesenje": r"$f'(x)=\tfrac{(5x+3)(x+1)}{2\sqrt{x+2}}$. Raste na $(-2,-1)$ i $(-\tfrac{3}{5},\infty)$, opada na $(-1,-\tfrac{3}{5})$. Lok. max u $x=-1$, lok. min u $x=-\tfrac{3}{5}$.",
    ...
    },
    {
        ...
        "rjesenje": r"$f''(x)=16x(x+1)e^{-2x^{2}-2x+1}$. Konveksna na $(-\infty,-1)$ i $(0,\infty)$, konkavna na $(-1,0)$. Infleksije: $x=-1$ i $x=0$.",
    ...
    },
    {
        ...
        "tocan_odgovor": multi([
            {"label": "Lokalni maksimum u x =", "answer": "1"},
            {"label": "f(lok. max) =", "answer": "2"},
            {"label": "Lokalni minimum u x =", "answer": "-1"},
            {"label": "f(lok. min) =", "answer": "-2"}
        ]),
    ...
    },
    {
        ...
        "tocan_odgovor": multi([
            ...
            {"label": "f(lok. min) =", "answer": "e^2/16"},
            {"label": "Točka infleksije u x =", "answer": "e^6"}
        ]),
        "rjesenje": r"$f'(x)=\tfrac{\ln x - 4}{2\sqrt{x}(\ln x)^{3}}$. Opada na $(1,e^{4})$, raste na $(e^{4},\infty)$. Min u $x=e^{4}$: $f(e^{4})=\tfrac{e^{2}}{16}$. Infleksija u $x=e^{6}$.",
